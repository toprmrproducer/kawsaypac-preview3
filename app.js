...
if(matchMedia('(prefers-reduced-motion: reduce)').matches){$$('.reveal').forEach(e=>e.classList.add('visible'));return}const io=new IntersectionObserver(es=>es.forEach(e=>{if(e.isIntersecting){e.target.classList.add('visible');io.unobserve(e.target)}}),{threshold:.12});$$('.reveal').forEach(e=>io.observe(e));
  ...
  function initJournalLinks(){const footer=$$('.footer-col').find(col=>col.querySelector('h3')?.textContent.trim()==='Learn');if(footer&&!footer.querySelector('a[href="journal.html"]'))footer.querySelector('ul')?.insertAdjacentHTML('beforeend','<li><a href="journal.html">Blogs</a></li>')}
  ...
  function renderRitualGallery(){
    const gallery=$('[data-image-stories]');if(!gallery)return;
    const scenes=[
      ['generated/rituals/ritual-morning-jar.webp','Hold the whole plant close.','Morning · jar and cup'],
      ['generated/rituals/ritual-brewing-jar.webp','Measure slowly. Brew with attention.','Preparation · loose-leaf ritual'],
      ['generated/rituals/ritual-evening-jar.webp','A warm cup can mark the day changing pace.','Evening · quiet window light'],
      ['generated/rituals/ritual-shared-jar.webp','Some rituals are better passed hand to hand.','Together · golden-hour veranda']
    ];
    gallery.innerHTML=scenes.map((scene,index)=>`<figure class="image-story reveal" data-ritual-index="${String(index+1).padStart(2,'0')}"><img loading="lazy" decoding="async" width="1122" height="1402" src="assets/img/${scene[0]}?v=105" alt="${scene[2]}"><figcaption class="image-story-copy"><blockquote>${scene[1]}</blockquote><cite>${scene[2]}</cite></figcaption></figure>`).join('');
  }
  function initRitualGallery(){
    const gallery=$('[data-image-stories]'),prev=$('[data-ritual-prev]'),next=$('[data-ritual-next]'),current=$('[data-ritual-current]');
    if(!gallery||!prev||!next||!current)return;
    const cards=$$('.image-story',gallery);
    const step=()=>{const gap=parseFloat(getComputedStyle(gallery).gap)||0;return(cards[0]?.getBoundingClientRect().width||gallery.clientWidth*.8)+gap};
    const update=()=>{const index=Math.max(0,Math.min(cards.length-1,Math.round(gallery.scrollLeft/step())));current.textContent=String(index+1).padStart(2,'0')};
    const move=direction=>gallery.scrollBy({left:direction*step(),behavior:'smooth'});
    prev.addEventListener('click',()=>move(-1));
    next.addEventListener('click',()=>move(1));
    gallery.addEventListener('scroll',()=>requestAnimationFrame(update),{passive:true});
    gallery.addEventListener('keydown',event=>{if(event.key==='ArrowLeft'){event.preventDefault();move(-1)}if(event.key==='ArrowRight'){event.preventDefault();move(1)}});
    update();
  }
  ...
  function boot(){renderHeader();normalizeRibbon();renderFooter();renderModal();initJournalLinks();renderHomeData();renderRitualGallery();initRitualGallery();initDraggableSprites();initNav();initNavSearchCart();initHomeVideo();initHero();initFilm();initReveal();initForms();initModal();initLivingInterface();initPhilosophySpotlight();initPopovers();initFooterHummingbird();initStatCounters();initConcernScroll();initSectionDrift();initPageNavigator();try{console.log('%cBrewed by hand in Ecuador. Curious minds welcome.','color:#1F3A2A;font-size:13px;font-family:Georgia,serif')}catch(e){}document.documentElement.classList.add('ready')}
